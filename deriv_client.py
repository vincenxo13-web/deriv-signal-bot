"""
Deriv public WebSocket client for signal-only market data.

No account token, authorize call, proposal, buy, or MT5 execution is used here.
The bot only streams prices and sends Telegram alerts.
"""
...
class DerivWebSocketClient:
    """Thin async websocket wrapper with reconnect and public tick subscriptions."""
    ...
    async def _subscribe_ticks(self, ws: Any, symbols: list[str]) -> None:
        for sym in symbols:
            ...
            await self._send_json(ws, {"ticks": sym, "subscribe": 1, "req_id": req_id})
    ...
    async def stream_ticks(self, symbols: list[str], on_message: MessageHandler) -> None:
        """Long-running public tick stream."""
        self._running = True
        backoff = 1.0
        ...
        while self._running:
            try:
                async with ws_connect(uri, ping_interval=None, max_queue=512) as ws:
                    self._ws = ws
...
async def fetch_active_symbols_brief(settings: Settings | None = None) -> list[str]:
    """Pull brief active symbol list via public websocket."""
    ...
    async with ws_connect(uri, ping_interval=None) as ws:
        ...
        for row in data.get("active_symbols") or []:
            sym = row.get("symbol")
            if sym:
                symbols.append(sym)
